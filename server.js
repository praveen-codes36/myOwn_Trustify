...
app.post('/add-source', async (req, res) => {
    try {
        ...
        if (!content || content.length < 100) {
            return res.status(400).json({ error: "Source content too short or unreachable." });
        }
        ...
        res.json({ message: `Successfully added ${insertedCount} chunks from ${url}` });
    } catch (error) {
...
app.post('/chat', async (req, res) => {
    try {
        // 1. Get sessionId (default to 'guest' if not sent)
        ...
        const newHistory = [
            ...history,
            { role: "user", parts: [{ text: message }] }, // Store simple user message
            { role: "model", parts: [{ text: finalText }] } // Store AI response
        ];

        // Optional: Keep history size manageable (last 20 turns)
